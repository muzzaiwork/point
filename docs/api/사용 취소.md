...
sequenceDiagram
    ...
    Service->>DB: 사용 상세 내역(PointDetail) 중 USE 타입 조회
    DB-->>Service: List<PointDetail> 반환
    ...
    loop 남은 취소 금액 소진 시까지
        ...
        alt 만료 안 됨
            ...
            Service->>DB: 신규 적립 생성 (Point, detailType=ACCUMULATE)
        end
        Service->>DetailEntity: PointDetail 생성 (detailType=USE_CANCEL)
    end
...
| **POINT_DETAIL** | (신규) | - | `amount: 1100 (detailType=USE_CANCEL)` | 상세 취소 내역 저장 |
...
5. **추적성 유지**: 어느 적립 건(Point)이 복구되었는지 혹은 신규 적립되었는지를 상세 내역(`PointDetail`)을 통해 1원 단위까지 관리합니다.
