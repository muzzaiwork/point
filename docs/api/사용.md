...
| `accumulatedPoint` | Long | O | 사용 금액 | `500` |
...
sequenceDiagram
    ...
    Client->>Controller: 포인트 사용 요청 (userId, orderNo, accumulatedPoint)
    Controller->>Service: use()
    Service->>DB: 사용자 조회 (Pessimistic Lock)
    DB-->>Service: UserAccount 객체 반환
    Service->>UserAccountEntity: usePoint(accumulatedPoint)
...
| **ORDER** | (신규) | - | `totalAmount: 1200, type: PURCHASE` | 사용 마스터 레코드 생성 |
