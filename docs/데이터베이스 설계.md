...
erDiagram
    ...
    POINT_EVENT {
        ...
        varchar pointEventType "이벤트 타입 (ACCUMULATE, ACCUMULATE_CANCEL, USE, USE_CANCEL, EXPIRE, ACCUMULATE)"
        bigint amount "금액"
        timestamp regDateTime "등록 일시"
        date regDate "등록 일자"
        timestamp updDateTime "수정 일시"
...
4. **POINT_EVENT (포인트 이벤트 이력)**
    ...
    - 이벤트 타입: `ACCUMULATE`(적립), `ACCUMULATE_CANCEL`(적립취소), `USE`(사용), `USE_CANCEL`(사용취소), `EXPIRE`(만료), `ACCUMULATE`(만료후재발급)
