...
- **핵심 포인트**
  ...
  2. **데이터 정합성 — 동시성과 멱등성**
     ...
     - **DB 비관적 락(`SELECT FOR UPDATE`)** 으로 동일 사용자에 대한 요청을 직렬화하여 잔액 정합성을 보장했다. Redis 분산 락도 고민했으나, DB락으로 진행. → [📄 동시성 제어 상세 보기](docs/동시성%20제어.md)
...
6. [아키텍처 구성](#-6-아키텍처-구성)
7. [무신사 페이먼츠 도메인 구조 추측](#-7-도메인-구조-추측)
...
> 💡 **테스트 데이터 안내**  
> 편의를 위해 애플리케이션 최초 실행 시 자동으로 테스트 데이터를 삽입합니다.
...
sequenceDiagram
    ...
    Client->>Controller: 포인트 적립 요청 (userId, amount, pointSourceType, type, expiryDays, orderNo)
...
|----|----------|----------|-----------------|----------------|--------|
| 1 | 1 | null | null | ACCUMULATE | 1000 |
| 2 | 1 | null | null | ACCUMULATE_CANCEL | 1000 |
...
고객 결제
   │
   ├─ [1] 포인트 차감  (Point 잔액 감소, PointEvent USE 기록)
   │
   └─ [2] PG 승인 요청
               │
               ▼
          PG사 (카드사 / 간편결제)
               │ 승인 완료 (Webhook → Musinsa Payments)
               ▼
          Musinsa Payments 수신
               │
               ├─ 결제 금액 확정  ([1] 포인트 금액 + [2] PG 금액)
               │  ※ 구매 확정 시 유료 포인트(PAID) 적립이 발생할 수 있음
               │
               ▼
          Settlement Engine
               │  일별 / 월별 집계
