...
<details>
<summary><strong>4. ORDER_CANCEL (취소 이력)</strong></summary>
...
**설계 포인트**
- 부분 취소가 여러 번 발생할 수 있으므로 `ORDERS`와 1:N 관계입니다.
...
| idx_order_cancel_reg_date_time | regDateTime | 일자별 취소 통계 집계 (일시) |
| idx_order_cancel_reg_date | regDate | 일자별 취소 통계 집계 (날짜) |
