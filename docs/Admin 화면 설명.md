...
총 적립 또는 총 사용 금액을 클릭하면 해당 사용자의 포인트 적립 내역 또는 사용 내역 페이지로 바로 이동합니다.
...
상태(취소 여부, 만료 여부), 타입(유료/무료), 출처(적립/수기지급/만료후취소재적립) 외에도 `pointKey`, `주문번호(orderNo)`, `루트 포인트` 키로 직접 조회할 수 있습니다.

![포인트-적립내역](images/포인트-적립내역.png)
...
| pointKey | 포인트 고유 키로 단건 직접 조회 |
| 주문번호 | 해당 주문에서 사용·취소된 연관 포인트 전체 조회 |
| 루트 포인트 | 재지급 체인 전체 조회 (rootPointKey 기준) |
| 취소 여부 | 전체 / 취소된 건만 필터링 |
...
| 컬럼 | 설명 |
|------|------|
| pointKey | 포인트 고유 키. 클릭 시 이벤트 이력 모달 팝업 |
| 사용자 ID | 포인트 소유자 |
...
| 만료 금액 | 만료된 금액 |
| 루트 포인트 | 재지급 체인의 최상위 포인트 키. 클릭 시 연관 포인트 내역 모달 팝업 |
| 원천 포인트 | 만료 후 재지급된 경우 직접 원천이 된 포인트 키 |
| 재지급된 포인트 | 이 포인트에서 재지급된 포인트 키 |
| 만료일 | 포인트 만료 예정일 |
| 적립일시 | 포인트 적립 일시 |
...
이벤트 타입 select로 특정 타입만 필터링할 수 있습니다.  
USE(사용) 또는 USE_CANCE(사용취소) 이벤트의 주문번호를 클릭하면 해당 주문의 사용 상세 또는 취소 상세 모달이 이벤트 이력 모달 위에 겹쳐서 열립니다.

![포인트-이벤트-이력-모달](images/포인트-이벤트-이력-모달.png)
![포인트-사용상세-모달](images/포인트-사용상세-모달.png)
![포인트-취소상세-모달](images/포인트-취소상세-모달.png)
...
`루트 포인트` 키를 클릭하면 동일 재지급 체인에 속한 모든 포인트(원천 포인트 + 재지급 포인트)를 한 화면에서 확인할 수 있습니다.  
...
사용 금액 또는 취소 금액 셀을 클릭하면 해당 주문의 포인트별 상세 내역을 모달로 확인할 수 있습니다.
...
| 취소 금액 | 취소된 포인트 총액. 클릭 시 취소 이력 및 포인트별 취소 상세 모달 팝업 |
| 주문일시 | 주문 생성 일시 |
| 취소일시 | 취소 처리 일시 |
...
사용 금액을 클릭하면 해당 주문에서 어떤 포인트가 얼마씩 차감되었는지 건별로 확인할 수 있습니다.  
취소 금액을 클릭하면 취소 이력과 함께 포인트별 복원 금액 상세를 확인할 수 있습니다.
...
각 금액은 `+`/`-` 부호와 색상으로 증감 방향을 직관적으로 표시하며, 우측 합계(SUM) 컬럼에서 기간별 순증감을 한눈에 파악할 수 있습니다.
