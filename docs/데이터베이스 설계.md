...
erDiagram
    ...
    ORDER ||--|{ POINT_USAGE_DETAIL : "consists_of"
    ...
    POINT {
        bigint id PK
        varchar userId FK "사용자 ID (USER.userId 참조)"
        varchar pointKey UK "적립 고유 키 (예: 20260331000001)"
        varchar orderNo "적립 근거 주문 번호 (선택 사항)"
        ...
        timestamp cancelledDate "적립 취소 일시"
    }

    ORDER {
        bigint id PK
        varchar userId FK "사용자 ID (USER.userId 참조)"
        varchar orderNo UK "주문 번호 (식별자)"
    ...
    POINT_USAGE_DETAIL {
        bigint id PK
        bigint order_id FK "ORDER 참조"
        bigint point_accumulation_id FK "POINT 참조"
        bigint amount "해당 적립 건에서 사용된 금액 (1원 단위 추적)"
        bigint cancelledAmount "해당 상세 내역에서 취소된 금액"
