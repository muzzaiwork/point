...
| 필드명 | 타입 | 필수 여부 | 설명 | 예시 |
| :--- | :--- | :--- | :--- | :--- |
| `cancelAmount` | Long | O | 취소 금액 (부분 취소 가능) | `500` |
...
sequenceDiagram
    ...
    Client->>Controller: 사용 취소 요청 (orderNo, cancelAmount)
    Controller->>Service: cancelUsage()
    Service->>DB: Order(사용 마스터) 조회
    DB-->>Service: Order 객체 반환
    Service->>OrderEntity: cancel(cancelAmount)
    Note over OrderEntity: 취소 가능 금액 검증 및<br/>canceledPoint 누적
    
    Service->>DB: OrderCancel 생성 및 저장
    
    Service->>DB: 사용자 조회 (Pessimistic Lock)
    DB-->>Service: UserAccount 객체 반환
    Service->>UserAccountEntity: cancelUsage(subAmount)
    
    Service->>DB: 사용 상세 내역(PointEvent) 중 USE 타입 조회
    DB-->>Service: List<PointEvent> 반환
    
    loop 남은 취소 금액 소진 시까지
        Service->>DB: 해당 USE 건에 대한 USE_CANCEL 합계 조회
        Service->>PointEntity: 적립 건(Point) 만료 여부 확인
        alt 만료 안 됨
            Service->>PointEntity: restore(subAmount)
            Service->>EventEntity: PointEvent 생성 (detailType=USE_CANCEL)
        else 이미 만료됨
            Service->>Service: createNewAccumulationForExpiredCancellation()
            Service->>PointEntity: 신규 적립 생성 (Point, pointSourceType=AUTO_RESTORED, rootId 유지)
...
| 테이블 | 필드 | 변경 전 | 변경 후 | 비고 |
| :--- | :--- | :--- | :--- | :--- |
| **USER_ACCOUNT** | `remainingPoint` | `300` | `1,400` | 전체 잔액 1,100P 복구 |
| **POINT (B)** | `remainingPoint` | `0` | `400` | B에서 사용한 500P 중 400P 복구 (미만료) |
| **POINT (신규)** | (신규 추가) | - | `accumulatedPoint: 700` | A에서 사용한 700P는 만료되어 신규 적립 |
| **ORDER** | `type` | `PURCHASE` | `PARTIAL_CANCEL` | 전체 취소 시 `TOTAL_CANCEL` |
| **ORDER** | `canceledPoint`| `0` | `1,100` | 주문 마스터에 취소 금액 누적 |
| **ORDER_CANCEL** | (신규) | - | `cancelAmount: 1100` | 취소 이력 레코드 생성 |
| **POINT_EVENT** | (신규) | - | `amount: 1100 (detailType=USE_CANCEL / ACCUMULATE)` | 상세 취소/재발급 내역 저장 |
...
| 테이블 | 필드 | 상태 | 결과 | 비고 |
| :--- | :--- | :--- | :--- | :--- |
| **ORDER** | `canceledPoint` | `1,100` | **취소 불가** | 취소 가능 금액(100) 초과 |
| **USER_ACCOUNT** | `remainingPoint` | `1,400` | **변화 없음** | 예외 발생 (400 Bad Request) |
...
3. **금액 검증**: 취소하려는 총 금액은 원본 사용 금액(`orderedPoint`)을 초과할 수 없습니다.
4. **이력 관리**: 취소 시마다 `OrderCancel` 레코드를 생성하여 취소 이력을 관리합니다.
5. **추적성 유지**: 어느 적립 건(Point)이 복구되었는지 혹은 신규 적립되었는지를 상세 내역(`PointEvent`)을 통해 1원 단위까지 관리합니다.
