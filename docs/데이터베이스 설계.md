...
erDiagram
    ...
    ORDER ||--o{ POINT_EVENT : "events"
    POINT ||--o{ POINT_EVENT : "events"
    ORDER_CANCEL ||--|| POINT_EVENT : "events"
    ...
    POINT {
        ...
        bigint rootPointId "전체 이력 추적을 위한 최상위 적립 ID"
    ...
    POINT_EVENT {
        ...
        varchar pointEventType "이벤트 타입 (ACCUMULATE, ACCUMULATE_CANCEL, USE, USE_CANCEL, EXPIRE)"
        bigint amount "금액"
        timestamp regDateTime "등록 일시"
        date regDate "등록 일자"
        timestamp updDateTime "수정 일시"
...
2. **POINT (적립 내역)**
    ...
    - `originPointKey` 필드로 재적립 시 직전 적립 건을 추적합니다.
    - `rootPointId` 필드로 최초 적립 건부터 이어지는 전체 이력을 추적합니다.
...
3. **ORDER (사용/주문 내역)**
    ...
    - `orderedPoint`는 주문 시 사용한 포인트로 불변이며, `canceledPoint`는 취소 시 누적됩니다.
    - `orderedPoint - canceledPoint`가 취소 가능 잔여 금액이며, 이를 초과하는 취소는 불가합니다.
    - `type` 필드로 주문의 성격(구매, 부분 취소, 전체 취소)을 구분합니다.
    - `status` 필드로 주문의 진행 상태(`IN_PROGRESS`, `CONFIRMED`)를 관리합니다.

4. **POINT_EVENT (포인트 이벤트 이력)**
    - 적립, 사용, 사용 취소, 만료, 재발급 등 모든 포인트 활동을 1원 단위로 기록합니다.
    - `pointEventType`을 통해 활동 성격을 구분하며, 적립 건(`POINT`) 및 주문 건(`ORDER`)과 연관되어 포인트의 흐름을 명확히 추적합니다.
    - 이벤트 타입: `ACCUMULATE`(적립/만료후재발급), `ACCUMULATE_CANCEL`(적립취소), `USE`(사용), `USE_CANCEL`(사용취소), `EXPIRE`(만료)
    - 출처 정보(`pointSourceType`)는 `POINT` 테이블에서 관리하며 `POINT_EVENT`에서는 중복 보관하지 않습니다.
