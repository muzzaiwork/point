...
sequenceDiagram
    ...
    Note over EventEntity: 설계상 적립 취소는<br/>포인트를 소모하는 행위이므로<br/>USE_CANCEL(또는 별도 감액 타입)로 기록
    
    Service->>UserAccountEntity: cancelAccumulation(cancelAmount)
    Note over UserAccountEntity: 적립된 금액을 회수하므로 전체 잔액(remainingPoint)에서 차감
    UserAccountEntity-->>Service: 사용자 잔액 업데이트 완료
    
    Service->>DB: Point, PointEvent & UserAccount 저장
...
| **POINT** | `remainingPoint` | `1,000` | `0` | 해당 적립 건의 사용 가능 잔액 0으로 변경 |
| **USER_ACCOUNT** | `remainingPoint` | `6,000` | `5,000` | **전체 잔액에서 1,000P 차감** (적립 철회) |
...
| 테이블 | 필드 | 상태 | 결과 | 비고 |
| :--- | :--- | :--- | :--- | :--- |
| **POINT** | `remainingPoint` | `800` | **취소 불가** | `accumulatedPoint(1,000)`와 불일치 |
...
2. **전체 잔액 반영**: 취소 시 사용자의 `remainingPoint`에서 취소되는 적립 건의 잔액만큼 차감합니다. (사용자가 적립받은 혜택을 회수하는 개념)
