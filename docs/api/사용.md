...
sequenceDiagram
    ...
    loop 사용 금액 소진 시까지
        ...
        Service->>EventEntity: PointEvent 생성 (detailType=USE)
    end
...
| **POINT_EVENT** | (신규 2건) | - | `B: 500, A: 700 (detailType=USE)` | 상세 매칭 내역 저장 (추적용) |
...
3. **추적성**: 포인트가 사용될 때마다 어떤 적립 건에서 얼마가 차감되었는지를 `PointEvent`에 1원 단위까지 기록합니다.
