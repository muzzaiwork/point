...
- **과제를 보며 중점으로 잡은 포인트**
  ...
  2. **정합성 — 유저 잔액에 발생하는 동시성 문제**
     - 동일 사용자에 대한 포인트 적립·사용 요청이 동시에 들어올 경우 `UserAccount`의 잔액(`balance`)이 오염될 수 있다.
     - **DB 락 vs Redis 락** 을 고민했고, 외부 인프라 의존 없이 정합성을 보장하기 위해 **DB 비관적 락(`SELECT FOR UPDATE`)** 을 선택했다.
     - Redis 분산 락은 처리량(Throughput)이 중요한 대규모 환경에서 유리하지만, 락 서버 장애 시 포인트 정합성 리스크가 생기므로 현 과제 범위에서는 DB 락이 더 안전하다고 판단했다.
