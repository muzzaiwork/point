...
/**
 * 애플리케이션 시작 시 1년치 더미 데이터를 삽입하는 초기화 컴포넌트.
 *
 * <p>{@code test} 프로파일에서는 동작하지 않는다 ({@code @Profile("!test")}).
 * 이미 데이터가 존재하면 초기화를 건너뛴다 (멱등성 보장).
 *
 * <p>생성 데이터:
 * <ul>
 *   <li>일반 유저 10명 ({@code seed01 ~ seed10}): 2025-01-01 ~ 2025-12-31, 하루 10건씩 10가지 시나리오 순환</li>
 *   <li>복잡 케이스 유저 3명 ({@code complex01 ~ complex03}): 만료 후 취소 → AUTO_RESTORED 반복 시나리오</li>
 * </ul>
 *
 * <p>시나리오 종류 (scenario 0~9):
 * <ol start="0">
 *   <li>단순 적립</li>
 *   <li>적립 후 즉시 적립 취소</li>
 *   <li>적립 후 사용</li>
 *   <li>사용 후 전액 취소</li>
 *   <li>사용 후 부분 취소</li>
 *   <li>만료 강제 처리 후 취소 → AUTO_RESTORED</li>
 *   <li>여러 건 적립 후 전액 사용</li>
 *   <li>수기 지급(MANUAL) + 사용</li>
 *   <li>복합: 적립 → 사용 → 사용 취소 → 재사용 → 만료 → 취소(AUTO_RESTORED)</li>
 *   <li>복합: 적립 → 사용 → 부분 취소 → 재사용 → 만료 → 취소(AUTO_RESTORED)</li>
 * </ol>
 */
@Slf4j
@Profile("!test")
