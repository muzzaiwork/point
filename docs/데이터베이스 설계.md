...
erDiagram
    ...
    POINT {
        ...
        timestamp accumulationDate "적립 일시"
        date accumulationDay "적립 일자"
        timestamp expiryDate "만료 일시"
        date expiryDay "만료 일자"
        boolean isCancelled "적립 취소 여부"
        timestamp cancelledDate "취소 일시"
        date cancelledDay "취소 일자"
        timestamp regDateTime "등록 일시"
        date regDate "등록 일자"
        timestamp updDateTime "수정 일시"
        date updDate "수정 일자"
...
- **ORDER (사용/주문 내역)**
    - `idx_order_user_id_usage_date`: `(userId, usageDate)`
        - 특정 사용자의 사용 이력을 최신순으로 조회할 때 유용합니다.
    - `idx_order_usage_date`: `(usageDate)`
