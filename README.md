...
- **구현 아젠다**
  ...
  1. **모든 포인트에는 적립 이후 종결까지 로직적으로 꼬리표가 달려있어야 한다.**
     ...
     - 사용 취소 시 만료된 포인트가 끼어있는 경우에도 재지급 이력을 남겨, 추적 고리가 끊기지 않도록 설계한다.

  2. **데이터 정합성 — 동시성과 멱등성**
     - 동일 사용자에 대한 포인트 적립·사용 요청이 동시에 들어올 경우 `UserAccount`의 잔액(`remainingPoint`)이 오염될 수 있다.
     - **DB 비관적 락(`SELECT FOR UPDATE`)** 으로 동일 사용자에 대한 요청을 직렬화하여 잔액 정합성을 보장했다. Redis 분산 락도 고민했으나, 외부 인프라 의존 없이 정합성을 우선하는 현 과제 범위에서는 DB 락이 더 적합하다고 판단했다. → [📄 동시성 제어 상세 보기](docs/동시성%20제어.md)
...
| id | point_id | pointEventType | amount |
|----|-----------------------|----------------|--------|
| 1 | 1 | ACCUMULATE | 1000 |
...
| # | 케이스 | 설명 | URL |
|---|--------|------|-----|
| 1 | 단순 적립, 미사용 | 적립만 되고 사용·취소·만료 이력이 없는 pointKey | [20260406000876](http://localhost:8080/admin/points?pointKey=20260406000876) |
| 2 | 사용 후 사용취소로 원복 | 일부 사용됐다가 USE_CANCEL로 잔액이 원복된 pointKey | [20260406000924](http://localhost:8080/admin/points?pointKey=20260406000924) |
| 3 | 만료된 pointKey | EXPIRE 이벤트가 기록된 만료 포인트 | [20260406000005](http://localhost:8080/admin/points?pointKey=20260406000005) |
| 4 | 만료 후 재지급 (AUTO_RESTORED) | 만료 후 사용취소로 AUTO_RESTORED 재지급된 pointKey | [20260406000006](http://localhost:8080/admin/points?pointKey=20260406000006) |
| 5 | 재지급이 여러 번 발생한 rootPointKey | AUTO_RESTORED가 6회 반복된 체인 | [20260406004625](http://localhost:8080/admin/points?rootPointKey=20260406004625) |
| 6 | 한 포인트를 여러 주문에 나눠 사용 | 적립 1건을 3개 orderNo에 분할 사용 후 일부 취소 | [complex04 포인트 조회](http://localhost:8080/admin/points?userId=complex04) |
...
> 이 시스템은 무신사 결제 도메인의 일부(포인트)를 구현한 것입니다.  
> 실제 서비스 전체 구조를 코드와 도메인 지식을 바탕으로 추측해보았습니다.
...
[User]
   │  구매 요청
   ▼
[Musinsa Platform]          ← 상품/주문/회원 관리
   │  결제 요청
   ▼
[Musinsa Payments]          ← 결제 수단 통합 처리
   ├─ PG (카드 / 카카오페이 / 네이버페이)
   ├─ Musinsa Pay           ← 선불 충전 머니
   └─ Point / Money         ← 포인트 & 머니 차감  ← 이 시스템
   │  정산 데이터 전달
   ▼
[Settlement Engine]         ← 매출 집계 / 수수료 계산 / 정산 스케줄링
   │  정산 지급
   ▼
[Seller]                    ← 입점 브랜드 / 셀러
```
...
### 7.3 이 시스템에 추가로 존재할 것 같은 기능 (도메인 추측)

| 기능 | 추측 근거 |
|------|-----------|
| 포인트 유효기간 연장 | 만료 임박 알림 + 연장 이벤트 프로모션 |
| 포인트 선물 / 이전 | 친구 초대, 리뷰 보상 등 마케팅 수단 |
| 등급별 적립률 차등 | VIP / 일반 회원 구분, 구매 금액 기반 등급 |
| 포인트 적립 알림 | 적립/만료 임박 Push / 카카오 알림톡 |
| 포인트 합산 한도 정책 | 1회 사용 한도, 주문 금액 대비 사용 비율 제한 |
| 관리자 수기 지급 / 회수 | CS 처리, 오지급 정정 (MANUAL 타입 이미 존재) |
| 포인트 적립 예약 | 구매 확정 후 N일 뒤 적립 (반품 기간 고려) |
| 정산 연동 | 포인트 차감분을 플랫폼이 부담하는 정산 처리 |
