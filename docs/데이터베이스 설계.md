...
erDiagram
    ...
    USER_ACCOUNT ||--o{ PT_ORDER : "use"
    PT_ORDER ||--o{ ORDER_CANCEL : "cancel"
    PT_ORDER ||--o{ POINT_EVENT : "event"
    ...
    POINT_EVENT {
        bigint id PK
        bigint order_id FK
        bigint point_accumulation_id FK
        bigint order_cancel_id FK
        varchar pointEventType
        bigint amount
        timestamp regDateTime
        date regDate
        timestamp updDateTime
...
| `MANUAL` | 수기 지급 (CS 처리 등), 사용 시 최우선 차감 |
| `AUTO_RESTORED` | 만료 후 취소로 인한 자동 재지급 |

**PointEventType** — `POINT_EVENT.pointEventType`
...
| `USE` | 포인트 사용 |
| `USE_CANCEL` | 사용 취소 (유효한 적립 건 복구) |
| `EXPIRE` | 포인트 만료 |
| `EXPIRED_CANCEL_RESTORE` | 만료된 적립 건의 사용 취소 시 신규 적립 |

**OrderType** — `PT_ORDER.type`
...
**OrderStatus** — `PT_ORDER.status`

| 값 | 설명 |
|----|------|
| `IN_PROGRESS` | 진행 중 (취소 가능) |
| `CONFIRMED` | 확정 (취소 불가) |
...
| rootPointKey | varchar | 최상위 적립 pointKey (전체 이력 체인 추적) |
